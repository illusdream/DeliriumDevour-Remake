...
namespace MagicaCloth2
{
    public class TeamManager : IManager, IValid
    {
        ...
        public struct TeamData
        {
            ...
            //public bool IsProcess => IsEnable && flag.IsSet(Flag_SyncSuspend) == false && flag.IsSet(Flag_CameraCullingInvisible) == false;
        ...
        //=========================================================================================
#if true
        static readonly ProfilerMarker teamCameraCullingProfiler = new ProfilerMarker("CameraCulling");
        ...
        /// </summary>
        internal void CameraCullingUpdate()
        {
            ...
            foreach (var cprocess in clothSet)
            {
                ...
                // 動作判定 ----------------------------------------------------
                if (tdata.IsEnable == false || tdata.flag.IsSet(Flag_SyncSuspend))
                    continue;

                //-------------------------------------------------------------
                // 現在の状態
                bool oldInvisible = tdata.IsCameraCullingInvisible;
                bool invisible;
        ...
        internal void AlwaysTeamUpdate()
        {
            ...
            // メインカメラ座標
            Vector3 mainCameraPosition = Vector3.zero;
            if (Camera.main)
            ...
            foreach (var cprocess in clothSet)
            {
                ...
                if (cloth.SyncCloth != null)
                {
                    ...
                    if (syncTeamId > 0)
                    {
                        ...
                        if (syncTeamData.IsEnable && cloth.SyncCloth.Process.GetSuspendCounter() == 0)
                ...
                // 距離カリング判定 -------------------------------------------------------------
                DistanceCullingUpdate(hasMainCamera, mainCameraPosition, cprocess, ref tdata);
        ...
        /// <summary>
        /// 実行検証
        /// 予期せぬ状態が発生した場合は実行を停止させる
        /// </summary>
        /// <param name="cprocess"></param>
        void ExecutionVerify(ClothProcess cprocess)
        {
            bool old = cprocess.IsState(ClothProcess.State_Verification);
            bool now = true;

            // スケール検証
            // ０スケールは動作不良を起こす
            float3 scl = cprocess.cloth.transform.lossyScale;
            scl = math.abs(scl);
            const float minScale = 0.000001f;
            if (scl.x < minScale || scl.y < minScale || scl.z < minScale)
                now = false;

            if (old != now)
            {
                cprocess.SetState(ClothProcess.State_Verification, now);
                cprocess.UpdateUse();
            }
        }
        ...
        /// 距離カリング判定
        /// </summary>
        /// <param name="hasMainCamera"></param>
        /// <param name="mainCameraPosition"></param>
        /// <param name="cprocess"></param>
        /// <param name="tdata"></param>
        void DistanceCullingUpdate(bool hasMainCamera, Vector3 mainCameraPosition, ClothProcess cprocess, ref TeamData tdata)
        {
            // 現在の状態
            ...
            // 距離カリングの有無
            if (jugeSettings.distanceCullingLength.use)
            {
                // 距離カリング有効
                // 距離判定
                Vector3 pos = jugeCloth.transform.position;
                Vector3 spos = jugeSettings.distanceCullingReferenceObject != null
                    ? jugeSettings.distanceCullingReferenceObject.transform.position
                    : mainCameraPosition;
                float dist = Vector3.Distance(pos, spos);
                float cullingDist = jugeSettings.distanceCullingLength.value;
                if (hasMainCamera == false && jugeSettings.distanceCullingReferenceObject == null)
                    cullingDist = 1000000.0f; // メインカメラnull時の安全対策
                invisible = dist >= cullingDist;

                // フェードウエイト
                float fadeDist = math.saturate(jugeSettings.distanceCullingFadeRatio) * cullingDist;
