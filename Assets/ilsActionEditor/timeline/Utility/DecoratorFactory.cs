    ...
    public static class DecoratorFactory
    {
        ...
        static T CreateDecorator<T>(Type targetType) where T : IDecorator {
            ...
            if ( !decoratorsTypeMap.TryGetValue(targetType, out decoratorType) ) {
                ...
                var decoratorTypes = Slate.ReflectionTools.GetImplementationsOf(typeof(T));
                for ( var i = 0; i < decoratorTypes.Length; i++ ) {
                    var current = decoratorTypes[i];
                    var att = Slate.ReflectionTools.RTGetAttribute<DecoratorAttribute>(current, true);
                    if ( att == null ) {
                    ...
                    if ( Slate.ReflectionTools.RTIsAssignableFrom(att.targetType, targetType) ) {
            ...
            if ( Slate.ReflectionTools.RTIsSubclassOf(decoratorType, typeof(ScriptableObject)) ) {
