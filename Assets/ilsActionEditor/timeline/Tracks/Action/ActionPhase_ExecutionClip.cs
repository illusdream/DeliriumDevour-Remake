...
{
    public class ActionPhase_ExecutionClip : BaseActionClip
    {
        [SerializeField]
        [HideInInspector]
        ...
        {
            //修改长度，使得可以一直衔接
            if (parent is ActionPhasesTrack ap && ap.StartUpClip&& ap.RecoveryClip)
            {
                ap.StartUpClip.endTime = startTime;
                
                ap.RecoveryClip.startTime =endTime;
                ap.RecoveryClip.endTime = ap.endTime;
            } 
            base.OnClipLengthChanged();
        }

        public override string info => "ExecutingPhase";
#endif
