...
{
    public class ActionPhase_StartUpClip : BaseActionClip
    {
        [SerializeField]
        [HideInInspector]
        ...
        {
            //修改长度，使得可以一直衔接
            if (parent is ActionPhasesTrack ap && ap.ExecutionClip && ap.RecoveryClip)
            {
                ap.ExecutionClip.startTime = endTime;
                ap.ExecutionClip.endTime = ap.RecoveryClip.startTime;
            } 
            startTime = parent.startTime;
            base.OnClipLengthChanged();
        }

        public override string info => "StartUpPhase";
#endif
    }
}