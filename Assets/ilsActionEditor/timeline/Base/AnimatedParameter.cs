...
namespace ilsActionEditor
{
    ...
    public class AnimatedParameter : IAnimatableData, ISerializationCallbackReceiver
    {
...
#if UNITY_EDITOR
            {
                if ( !Application.isPlaying && ilsActionEditor.Prefs.autoKey && Prefs.autoFirstKey && !HasAnyKey() ) {
                    TryKeyIdentity(0);
                }
            }
#endif
                ...
                if ( !Application.isPlaying ) {
                    if ( time == previousTime && HasChanged() ) {
                        if ( !Prefs.autoKey ) { //in case of no Auto-Key, store changed params
                            ...
                            var paramCallbacks = new ActionTimelineUtility.ChangedParameterCallbacks(restore, commit);
                            ActionTimelineUtility.changedParameterCallbacks[this] = paramCallbacks;
                        }
                        return; //auto-key or not, return if the parameter changed
                    }
                    if ( !Prefs.autoKey ) {
                        ActionTimelineUtility.changedParameterCallbacks.Remove(this);
                    }
                }
            }
#endif
        ...
        public void OffsetValue(object deltaValue) {
            ...
            for ( var i = 0; i < curves.Length; i++ ) {
                CurveUtility.OffsetCurveValue(curves[i], decomposed[i]);
            }
            NotifyChange();
        }
