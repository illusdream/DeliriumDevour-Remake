...
namespace ilsActionEditor.Editor
{
    public static class FieldFactory
    {
        ...
        static FieldFactory()
        {
            foreach (var fieldDrawerType in TypeCache.GetTypesWithAttribute(typeof(FieldDrawerAttribute)))
            {
                var attr = fieldDrawerType.GetCustomAttribute<FieldDrawerAttribute>();
                if (attr != null)
                {
                    AddDrawer(attr.fieldType, fieldDrawerType);
                }
            }
